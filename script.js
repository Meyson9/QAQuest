...
var startaStage = function startaStage() {
...
var togallVois = function togallVois() {
  var doc = document.querySelectorAll('.fab-buttons')[0].children;
  localButtonToggale('muteVoise', '.icon-material_li', 'icon-material_li_mute');
  localButtonToggale('nonstope', '.icon-material_tw', 'icon-material_tw_NonStop'); // режим без звука 

  doc[2].addEventListener('click', function (e) {
    e.preventDefault();
    toggaleButton('.icon-material_li', 'icon-material_li_mute', 'muteVoise'); // const icon = document.querySelector('.icon-material_li');
    // if (icon.classList.contains('icon-material_li_mute') && localStorage.getItem('muteVoise')) {
    //   icon.classList.remove('icon-material_li_mute')
    //   localStorage.removeItem('muteVoise')
    // } else {
    //   icon.classList.add('icon-material_li_mute')
    //   localStorage.setItem('muteVoise','voiseMute')
    // }
    // icon_material_li.classList.toggle(toggalClass)
    // const btn = document.querySelector('.icon-material_li');

    var icon_material_li = document.querySelector('.icon-material_tw'); // if(btn.classList.contains('icon-material_li_mute')){
    //   localStorage.setItem('muteVoise','voiseMute')
    // } else {
    //   localStorage.removeItem('muteVoise')
    // }
  ...
  doc[1].addEventListener('click', function (e) {
    e.preventDefault();
...
var whereStay = function whereStay() {
  ...
  doc[0].children[4].addEventListener('click', function (e) {
    ...
    if (sel) {
      ...
      var numberQuestion = sel.replace(/(tick|cross|heart|flower)(question)/, '') - 1; // if (selectorQuestionLevel === 'tick') {
...
var voiceQuestion = function voiceQuestion(iterator, vioseSkib) {
  ...
  var localMute = localStorage.getItem('muteVoise');
